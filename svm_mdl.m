...
labelout=[]; loss_vec=[]; cv_vec=[];
for k=1:5
    n=1;
    for i=5:length(train_inx{1,1})-1
        svm_mdl=fitclinear(train_inx{1,k}(1:i,:), targets_inx{1,k}(1:i,:),'Solver', 'dual',...
        'lambda', params.Lambda, 'LearnRate', params.LearnRate, 'PassLimit',params.PassLimit, ...
...
function plot_perform(loss_vec,valid_vec,n)
%plot performance - training error vs validation error
figure
plot(1:n,loss_vec)
hold on
plot(1:n,valid_vec,'m')
xlim([0 n]);  ylim([0 3]);
